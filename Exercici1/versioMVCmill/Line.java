...
public class Line extends Observable {
    ...
    public void moveRight() {
        if (pos < arr.size()) {
            super.setChanged();
            pos++;
            super.notifyObservers(new Console.ChangeWarn(Key.RIGHT));
    ...
    public void moveLeft() {
        if (pos > 0) {
            super.setChanged();
            pos--;
            super.notifyObservers(new Console.ChangeWarn(Key.LEFT));
        }
    }
    public void toHome() {
        if(pos>0){
            super.setChanged();
            pos = 0;
            super.notifyObservers(new Console.ChangeWarn(Key.HOME));
        }
    }
    ...
    public void toEnd() {
        ...
        setChanged();        
        pos = arr.size();
        notifyObservers(new Console.ChangeWarn(Key.END, str));
    ...
    public void sup() {
        if (pos < arr.size() - 1) { //si tenim el cursor pel mig
            super.setChanged(); 
            arr.remove(pos);
            super.notifyObservers(new Console.ChangeWarn(Key.SUP, getParteaEscribir(pos, arr.size()))); //le pasamos el array que debe sobresecribir

        } else if (pos == arr.size() - 1) {
            super.setChanged();
            arr.remove(pos);
            super.notifyObservers(new Console.ChangeWarn(Key.SUPLAST));
        }
    }
    ...
    public void del() {
        if (pos != 0) { //no estem a l'inici
            if (pos == arr.size()){  //estem amb el cursor a la dreta de l'ultim caracter
                super.setChanged();
                super.notifyObservers(new Console.ChangeWarn(Key.DELLAST));                           
            }

            else { // caracters pel mig
                super.setChanged();                
                super.notifyObservers(new Console.ChangeWarn(Key.DEL, getParteaEscribir(pos, arr.size())));
    ...
    public void write(char car) {
        ...
        super.setChanged();
        super.notifyObservers(new Console.ChangeWarn(Key.WRITE, ins, getParteaEscribir(pos, arr.size()), car));
